...
function initSocket(server) {
  ...
  let onlineUsers = new Map(); // Create a map to store active clients
  onlineUsers.clear();
  ...
  io.on("connection", (socket) => {
    ...
    socket.on(
      "addReaction",
      async ({ channelName, channelId, reactionType, usrId, msgId }) => {
        const existingReaction = await Reaction.findOne({
          where: { usrId, msgId },
        });

        if (existingReaction) {
          // if a reaction already exists, update the reactionType
          await existingReaction.update({ reactionType });
        } else {
          // if no reaction exists yet, create a new one
    ...
    socket.on(
      "deleteMessage",
      async ({ messageId, channelName, channelId }) => {
        try {
          ...
          io.to(socket.id).emit(
            "error",
            "you need to delete all the channels created by this user"
          );
        }
      }
    );
    ...
    socket.on("deleteChannel", async ({ channelId }) => {
      try {
        ...
        io.to(socket.id).emit(
          "error",
          "you need to delete all the Messages in this Channel first due to foriegn key constraint"
        );
      }
    });
    ...
    socket.on("searchMessages", async ({ text, channelId }) => {
      try {
        const messages = await Message.findAll({
          where: {
            to: channelId,
        ...
        // send the matched messages back to the client
        socket.emit("searchMessagesResult", messages);
      } catch (error) {
        // handle error
        console.error(error);
      }
